...
class train_pair_transform(object):
    ...
    def __call__(self, img, target, model):
...
#             x_crop_center = random.randint(min_x, max_x-1)
#             y_crop_center = random.randint(min_y, y_range_end-1)

#             y1, y2 = max(0, y_crop_center - self.final_h // 2), min(h_ori, y_crop_center + self.final_h // 2)
#             x1, x2 = max(0, x_crop_center - self.final_w // 2), min(w_ori, x_crop_center + self.final_w // 2)
#             x1 = random.randint(min_x, w - self.c_w)
#             y1 = random.randint(0, h - self.c_h)
#             self.last_crop_left = (x1,y1)
...
def createTrainData(datasetname, Dataset, cfg_data, distributed, model):
...
def loading_data(datasetname, val_interval, distributed, model, is_main):
    if 'MovingDroneCrowd' not in datasetname:
        datasetname = datasetname.upper()
    cfg_data = getattr(setting, datasetname).cfg_data

    Dataset = dataset.Dataset
    train_loader, sampler_train = createTrainData(datasetname, Dataset, cfg_data, distributed, model)
...
def createValTestData(datasetname, Dataset, cfg_data, frame_interval, skip_flag, is_main, mode ='val'):
    if is_main:
        ...
        target = True
        if 'MovingDroneCrowd' in datasetname:
            last_scene_names = []
            for scene_name in scene_names:
                root  = os.path.join(cfg_data.DATA_PATH, 'frames', scene_name)
                if '/' in scene_name:
                    scene_name, clip_name = scene_name.split('/')
                    clip_names = [clip_name]
                else:
                    clip_names = [clip_name for clip_name in os.listdir(root) if clip_name.isdigit()]
...
def createValTestData2(datasetname, Dataset, cfg_data, frame_interval, is_main, mode ='val'):
    if is_main:
        img_transform = standard_transforms.Compose([
            standard_transforms.ToTensor(),
            standard_transforms.Normalize(*cfg_data.MEAN_STD)
        ...
        with open(os.path.join(cfg_data.DATA_PATH, eval('cfg_data.{}_LST'.format(mode.upper()))), 'r') as txt:
            ...
            scene_names = [i.strip() for i in scene_names]
        target = True
        if datasetname == 'HT21' and mode == 'test':
            target = False
        if 'MovingDroneCrowd' in datasetname:
            last_scene_names = []
            for scene_name in scene_names:
        ...
        for scene_name in scene_names:
            ...
            sub_dataset = Dataset(scene_name = scene_name,
                                ...
                                main_transform=main_transform,
                                img_transform=img_transform,
                                interval= frame_interval[0][scene_name]*frame_interval[1] if isinstance(frame_interval, tuple) else frame_interval,
                                target=target,
                                datasetname = datasetname)
            sub_loader = DataLoader(sub_dataset, 
                                    batch_size=cfg_data.VAL_BATCH_SIZE, 
