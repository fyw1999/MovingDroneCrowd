...
class Dataset(data.Dataset):
    ...
    Dataset class.
    """
    def __init__(self, txt_path, base_path, main_transform=None, img_transform=None, train=True, datasetname='Empty', frame_intervals=(3,6), model="GD3A"):
        ...
        for scene_name in scene_names:
            if datasetname == 'SENSE':
                img_path, label = SENSE_ImgPath_and_Target(base_path, scene_name.strip())
            elif 'MovingDroneCrowd' in datasetname:
    ...
    def __getitem__(self, index):
        ...
        img0, target0 = self.main_transforms(img0, target0, self.model)
        img1, target1 = self.main_transforms(img1, target1, self.model)
        ...
        if not ((np.array(count_in_pair) > 3).all() and torch.sum(share_mask0) > 2):
...
class TestDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.valid[index]: # Avoid unnecessary file reading to save time
            index1 = index
            ...
            if self.target:
                ...
                target1['ori_width'], target1['ori_height'] = img1.size
                target2['ori_width'], target2['ori_height'] = img2.size
