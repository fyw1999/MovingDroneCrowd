...
class ResNet50FPN_Stride8(nn.Module):
    def __init__(self, pretrained=True):
        ...
        # 1. Load the ImageNet-pretrained ResNet50 backbone
        print(f"Loading ResNet50 ImageNet weights: {pretrained}")
        # Use the current torchvision API (fall back to models.resnet50(pretrained=True) if needed)
        weights = models.ResNet50_Weights.DEFAULT if pretrained else None
        resnet = models.resnet50(weights=weights)

        # Extract the ResNet stages
        ...
        # 2. Build the FPN modules (Conv + BN + ReLU)
        fpn_dim = 256

        # Top layer (processes C5)
        ...
        # Lateral layers (process C4, C3, and C2)
        ...
        # Smoothing layers (reduce aliasing)
        ...
        # 3. Final fusion layer (fuse to 256 channels)
        # Input: concatenate four pyramid levels (256*4 = 1024) -> output: 256
        ...
        # Initialize weights (the backbone is pretrained, so only the new layers need initialization)
        self._init_new_layers()

    def _init_new_layers(self):
        # Initialize only the non-backbone components
    ...
    def forward(self, x):
        ...
        # p3_smooth: Stride 8
        # p4_smooth: Stride 16
        # p5:        Stride 32 (p5 usually has no smoothing convolution, or one is added before output; reuse it directly here)

        # --- Multi-scale Fusion (Target: Stride 8) ---
        
        # 1. P2 (Stride 4) -> downsample -> Stride 8
        # Downsample using average pooling
        feat_s8_p2 = F.avg_pool2d(p2_smooth, kernel_size=2, stride=2)

        # 2. P3 (Stride 8) -> unchanged -> Stride 8
        feat_s8_p3 = p3_smooth

        # 3. P4 (Stride 16) -> upsample by 2x -> Stride 8
        feat_s8_p4 = F.interpolate(p4_smooth, scale_factor=2, mode='bilinear', align_corners=False)

        # 4. P5 (Stride 32) -> upsample by 4x -> Stride 8
